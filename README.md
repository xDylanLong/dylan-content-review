...
> 一个面向抖音和小红书的中文内容 Skill，把选题、标题、开头、逐字稿、共鸣和 AI 味检查，整理成可以直接执行的内容工作流。
>
> 中文内容诊断 · 短视频口播 · 小红书标题 · 内容复盘 · Codex Skill
...
`dylan-content` 把这些判断拆成六个可按需调用的模块。它以用户提供的原句、事实和素材为证据，输出具体的删改位置、替代句、内容骨架或下一步动作，不承诺完播率、点击率或“必火”。
...
```text
Use $dylan-content 诊断这篇抖音口播稿，并给出可以直接修改的版本。
```

### 本地开发绑定

本项目的本地源目录是 `/Users/thawingx/Documents/dylan-content`。当前 Codex 运行时目录 `/Users/thawingx/.codex/skills/dylan-content` 已绑定到该源目录，因此修改源目录后，运行时读取的 Skill 会同步更新。
