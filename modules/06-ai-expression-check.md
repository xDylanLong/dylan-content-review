...
本模块默认任务仍然是识别，不在用户没有要求时直接改稿。被 `dylan-series-speaking` 或其他口播生成流程调用时，使用下面的规则做交付前质检：

### 排比

- 一篇只允许一组有明确作用的排比，一般不超过 3 项；课程型讲解若每项都对应一个具体判断，可以到 4 项。
...
- 只有用户明确要求形象化表达，或抽象机制确实无法用具体动作和因果解释时才考虑；900–1100 字口播最多一个，且交付前再判断能否删掉。
...
1. 圈出所有“第一、第二、第三”“一是、二是、三是”和并列四项以上的句子。
2. 圈出所有比喻、虚构人物和“后来我才发现”式故事。
