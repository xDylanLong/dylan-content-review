...
- 一篇只允许一组有明确作用的排比，一般不超过 3 项；课程型讲解若每项都对应一个具体判断，可以到 4 项。900–1100 字口播最多 1 组；1600–1900 字的“1 分钟系列”等长稿最多 1 组，不因为字数多就默认放宽到 2 组。
- 三项必须有轻重差异，不能都写成“名词 + 动词 + 结果”的同一模具。
- 如果排比只是把观点拆成几个漂亮词，改成一个场景、一个动作和一个判断。
- “三点”“四个条件”“五个步骤”是学习结构，不等于口播里必须逐项念出来。正文可以用叙述把结构藏起来。
- **排比不只是“第一、第二、第三”。** “先看 A。再看 B。最后看 C。”“先说……，然后……，最后……”这类用近义词替换掉序数词的分段转场，只要三段内部结构、节奏一一对应，同样按一组排比计数，不能因为没写“第一/第二/第三”就当它不存在。
- **排比也可能出现在段落层面，不只是句子层面。** 如果一篇稿子对三个并列对象（例如三个维度、三个原因）分别用“提问或场景 → 解释这一层是什么 → 一个例子 → 收尾判断”这套完全相同的内部节拍展开，即使每段字数、用词不同，这三段读下来的“节奏感”仍然是排比——判断标准是“删掉具体名词后，这三段的骨架是不是同一张模板”。命中时至少改掉其中一段的内部顺序，或删掉其中一段的某个环节，让三段不再互为镜像。
- 全文如果同时存在两组排比（例如正文中段一组“三层拆解”，结尾又出现一组“三种解法”），只保留信息量更大的一组，另一组改写成连续叙述或合并进已有的那一组，不能因为出现在不同位置就分别按“各自都不超标”计数。

### 比喻和故事

- 默认不使用比喻，不承担开场、转场和结尾的装饰作用。
- 只有用户明确要求形象化表达，或抽象机制确实无法用具体动作和因果解释时才考虑；900–1100 字口播最多一个，且交付前再判断能否删掉。1600–1900 字的“1 分钟系列”等长稿最多 1–2 个，同样要在交付前逐个确认是否可删。
- 没有用户提供的经历或可靠来源时，不能写“我有个朋友”“有个客户”“某位创始人后来发现”这类伪故事。需要案例时，明确写“假设一个场景”。
- **同一意象重复出现也算比喻超标。** “拦住/拦下来”“掉链子”“打退堂鼓”“打架”这类表示“受阻、失败、对抗”的比喻或俗语，哪怕每次用词不同，只要在同一篇里反复用来指同一类状态，就是把一个比喻当成贯穿全文的修辞骨架在用，必须按“已用比喻数 +1”累计计数，不能因为换了说法就重新从 0 开始算。命中两次以上，只保留最贴切的一次，其余改成直接陈述（例如“时间和距离先把这件事拦住了”改成“时间和距离先不允许”“这一步先做不了”）。
...
排比和比喻在生成时很难自己发现——写的时候每一句都觉得合理，只有摆在一起数才看得出超标。所以交付前必须先产出两张清单，再决定要不要改，不能跳过清单直接凭感觉说“看起来没问题”：

1. **排比组清单**：逐条列出全文里所有“三项及以上、结构相互呼应”的地方，包括用序数词的（“第一、第二、第三”）、用近义词转场的（“先看/再看/最后看”“先说/然后/最后”）、以及段落级的（多段内部节拍相同）。每条写出命中的原文。清单条数 > 1 组，必须合并或改写到只剩 1 组。
2. **比喻／习语清单**：逐条列出全文里所有比喻、拟人和“受阻/失败/对抗”类习语（含“拦住”“掉链子”“打退堂鼓”“打架”等），同一意象出现多次要分别列出、按出现次数计数，不能合并成一条。900–1100 字稿清单超过 1 条、1600–1900 字稿超过 2 条，必须删减到上限内。
3. 圈出所有虚构人物和“后来我才发现”式故事。
4. 检查是否每段末尾都像结论，是否连续使用相同长度的句子。
5. 能用具体场景、动作或因果替换的，直接替换；替换后不足目标字数，补充边界和练习，不补空洞金句。
6. 如果出现“反直觉观点”“反常识观点”“容易被忽略的信号”等结构提示词，先检查它后面是否真的有案例、对比和新判断；口播稿优先删除提示词，保留内容推进。
7. 找到 Hook 之后第一个具体例子或第一个结论，按 `modules/02-hook-optimization.md` 的“正文第一个例子／第一个结论必须有冲击力”标准检查：这句话观众能不能提前猜到？不能命中数字冲击、反差、金句或具体细节里的任何一项，判定为不合格，必须换更有落差的角度。
