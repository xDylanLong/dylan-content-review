# Dylan Content Review：小红书标题公式工具
...
6. **方法名、模型名和理论名默认不做标题入口。** 除非内容是在直接介绍某个方法、模型或理论本身是什么，否则不要机械把它的名称放进标题；优先写受众的问题、场景、结果或冲突。它只是正文中的分析工具，正文可以提到并使用。需要保留名称时，让问题先行，例如“用福格行为模型分析一下，为什么你的产品没人用”；这里的福格模型只是示例，不是唯一适用对象。

对陌生模型、方法或理论，即使本篇是在介绍“它是什么”，也默认执行“宽问题 → 具体对象 → 模型名称”的入口层级：标题先写大众问题，封面写更短的价值点，前 2 秒延续问题，前 5 秒落到 AI 产品或具体场景，正文再揭示术语。若标题直接出现术语，必须能说明受众已普遍认识它，或用户明确要求术语检索型标题。

**标题必须同时具备场景／痛点和方法暗示，不能只是一句孤立的疑问句：**

- 场景／痛点：标题要落到一个具体、可代入的处境或结果（“健身卡半年去两次”），不能只是抽象话题（“如何坚持运动”）。
- 方法暗示：即使不点名模型，也要让受众感觉“这背后有一套能查、能拆的方法”，不是单纯的个人吐槽或猜谜。常见做法是用数字锚定（“3 个问题”“3 层原因”）或诊断类动词（“查”“拆开看”“分层排查”）暗示存在系统性方法；纯反差疑问句（“为什么……却……”）如果不配合任何数字或诊断动词，容易被当成情绪吐槽，看不出有方法论价值，应优先换成带方法暗示的版本，或在同一批候选里搭配至少一个带方法暗示的选项。
- 这条对系列内容尤其重要：观众刷到一条系列视频，标题里的方法暗示（哪怕只是“3 个问题”）能让人知道“这是系列的框架化内容”，不是孤立的个人经历分享；具体模型名称仍然不出现在标题里，留到正文揭晓。

### 标题生成与改写补充规则

- 介绍 Skill、工具、产品、方法或开源项目时，标题第一认知位先说清它用来做什么、能带来什么结果；项目名、内部代号或视觉 IP 不能单独承担标题入口，除非受众已经普遍认识它。需要保留名称时，放在用途描述之后。
- 标题、封面和 Hook 形成宽到窄的连续链：标题先提出普遍问题，封面补短价值点，前 2 秒延续问题，前 5 秒收窄到 AI 产品或具体工作场景，正文再引出模型名。不要让标题直接变成只有模型名的垂直课程标签。
- 数字、时间、数量和效果承诺必须有用户提供的事实或已验证的工作流程支撑；没有证据时，不擅自写死具体数量或确定效果。
- 不使用“最全、顶级、必看、彻底、100%”等无法兑现的承诺。标题要能直接念，不依赖封面才能理解，也不为了“像爆款”硬套公式。

### 标题质量 5 条铁律（来自全平台标题实战经验）

7. **标题留悬念，不说答案。** 用「为什么」不用「证明」，用「问题」不用「结论」。检查方法：看完标题，观众还需要点进来吗？如果标题已经把答案说完了，就是失败的标题。
8. **话题要扩大，不要缩窄。** 用普世词（赚钱、涨粉、不上班）替换行业术语（知识付费、引流款、自媒体）。缩窄话题 = 缩小算法推送的流量池。
9. **击中真实痛点，不是表象概念。** 「一人公司」是表象，「不上班」才是真实痛点；「消费型兴趣」是表象，「靠兴趣赚不到钱」才是真实痛点。问自己：受众真正想要的是什么？
10. **标题张力检查：至少满足 2 项。** 有对比/反差、有具体数字、有悬念/好奇心、有冲突/争议性、有时间承诺、有结果承诺——6 项至少命中 2 项，否则标题太平。
11. **小红书标题 ≤ 20 字（含标点）。** 硬性规范，标点符号也算字数，超了就砍。

### 视频脚本标题生成的补充规则（被 dylan-content-creation 调用时）

当本模块为视频口播项目生成或校验标题时，按以下加严格式执行，替代 Step 4 的默认输出格式：

1. **候选数量：5–10 个，覆盖至少 3 种公式类型。** 不用 Step 4 默认的 5–8 个下限，取上限区间，保证有足够候选可供筛选。
2. **每个候选必须 ≤ 20 字（含标点），逐字数出实际字数标注在候选后面。** 这是规则 11 的强制执行版本：候选生成阶段就按 20 字上限约束，不是先写长标题再事后砍；小红书是所有平台里标题字数要求最严的，也是标题本身权重最高的平台，字数校验不能省略。
3. **候选集里至少要有一半带方法暗示（数字锚定或诊断类动词），不能全是纯反差疑问句。** 每个候选都要落到具体场景／痛点，见上方“标题必须同时具备场景／痛点和方法暗示”；纯疑问句候选最多保留 1–2 个作对照，不能是候选集的主体。
4. **每个候选附三段分析，缺一不可：**
   - **为什么选这个（公式与钩子）：** 对应哪个公式编号、命中哪种心理触发器，一句话说清。
   - **受众怎么看、为什么被吸引点进来：** 从目标受众刷到这条标题的第一反应出发，说清他们会怎么理解这句话、联想到自己的什么处境，以及具体是什么驱动他们点进来（好奇缺口、身份代入、损失焦虑等）；要写到这个标题的具体措辞上，不能只是重复触发器类型的名称。
   - **正文如何承接：** 标题打开的期待或悬念，正文第几段、用什么例子或判断去兑现。标题和正文必须一起工作——不能标题喊出一个承诺，正文却没有对应段落去兑现它；这一条要和前 5 秒 Hook、正文第一个例子的衔接保持一致，不能各写各的。
5. **输出末尾给一个推荐 + 一句话理由，**理由要说明它相对其他候选在"标题张力检查"（至少命中 2 项）、"场景／方法暗示"和"正文承接"三方面的优势，不能只写"更吸引人"这类空泛结论。
6. 本补充规则只调整候选数量、字数硬上限、场景与方法暗示要求和分析深度，不改变 Step 1–3 的匹配逻辑与上方核心规则；每个标题仍必须能追溯到具体公式编号。
